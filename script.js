// === Carmilla's Curse - script.js (enhanced experience) ===
...
function init() {
  ...
  // Load textures for each face.  Two textures for the front (day and night)
  // allow swapping between sleeping and watching states.
  ...
  // Create a materials array in order [right, left, top, bottom, front, back]
  const materials = [
    ...
    new THREE.MeshBasicMaterial({ map: frontNightTexture, color: 0xffffff }), // front (default night)
    new THREE.MeshBasicMaterial({ map: backTexture,   color: 0xffffff })  // back
  ];

  // Expose front textures globally for the Tiki puzzle
...
function solveFace(face, message){
  ...
  // Dim the solved face as a visual indicator
...
function renderTikiPuzzle(face){
  ...
  function updateDayNight(){
    const hVal = parseFloat(sunSlider.value);
    ...
    const brightness = midday ? 1.3 : 0.7;
    renderer.domElement.style.filter = `brightness(${brightness})`;
    const newMap = midday ? window.__frontDayTexture : window.__frontNightTexture;
    if (cube.material[4].map !== newMap) {
      cube.material[4].map = newMap;
      cube.material[4].needsUpdate = true;
    }
  }
  sunSlider.addEventListener('input', updateDayNight);
  updateDayNight();
...
/* existing code for other puzzles unchanged … */
